...
export function JournalHero() {
  ...
  return (
    <section className="relative isolate h-[90vh] min-h-[640px] w-full overflow-hidden">
      ...
      {/* Floating social toolbar — same glass style as the homepage
          Experience section, reused for consistency across the site */}
      <motion.div
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}
        className="absolute left-1/2 top-[68%] z-10 flex -translate-x-1/2 items-center gap-2 rounded-2xl border border-white/40 bg-white/10 p-2 shadow-lg backdrop-blur-md"
      >
      ...
      {/* Glass content card, bottom-anchored */}
      <motion.div
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7, delay: 0.1, ease: [0.22, 1, 0.36, 1] }}
        className="absolute bottom-0 left-1/2 z-10 w-full max-w-lg -translate-x-1/2 rounded-t-3xl border border-white/20 bg-dark/70 px-8 pb-8 pt-10 text-center backdrop-blur-md"
      >
