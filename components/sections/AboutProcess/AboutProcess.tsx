        ...
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-[1fr_260px_1fr] lg:gap-10">
          ...
          <div className="flex flex-col justify-between gap-16 pl-4 lg:pl-6">
            {leftSteps.map((step, i) => (
              <ProcessStepRow key={step.id} step={step} align="left" index={i} isLast={i === leftSteps.length - 1} />
            ))}
          </div>
...
<div className="mt-16 flex flex-wrap justify-center gap-4">
  {extraSteps.map((step) => (
    <div key={step.id} className="w-[240px]">
      <BeliefCard icon={step.icon} title={step.title} description={step.description} />
    </div>
  ))}
</div>
      </Container>
